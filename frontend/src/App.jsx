  ...
  return (
    <div className={`app-shell app-step-${step}`}>
      ...
      <main className={`content content-step-${step}`}>
        ...
        {step === 3 && (
          <section className={`panel ${isFreePackage ? "free-review-panel" : ""}`}>
            <div className="panel-header">
              ...
              <h1>
                {isFreePackage
                  ? freeBookingSubmitted
                    ? "Your free review is recorded"
                    : "Your quick case check is ready"
                  : paymentConfirmed
                    ? "Package duties started"
                  : paymentPending
              ...
              <p>
                {isFreePackage
                  ? "The free package gives you a first look at the likely scam type, the strongest warning signs, and immediate safety steps before you decide what to do next."
                  : paymentConfirmed
            ...
            {isFreePackage && (
              <div className={`free-result-card risk-${riskAnalysis.level.toLowerCase()}`}>
                <div className="free-result-copy">
                  <span className="free-chip">Free package result</span>
                  <h2>{riskAnalysis.category}</h2>
                  <p>{riskAnalysis.summary}</p>
                  <div className="free-result-meta">
                    <span>{riskAnalysis.level} risk pattern</span>
                    <span>{riskAnalysis.confidence}% review confidence</span>
                    <span>{selectedLocalTime || "Appointment selected"}</span>
                  </div>
                </div>

                <div
                  className="free-score-orbit"
                  style={{ "--score": riskAnalysis.score }}
                  aria-label={`Risk score ${riskAnalysis.score} out of 100`}
                >
            ...
            {isFreePackage && (
              <div className="free-benefit-grid">
                {[
                  ["Category match", `This looks most like: ${riskAnalysis.category}.`],
                  ["Warning signs", "The free check highlights the signals that need attention first."],
                  ["Safety steps", "You get simple actions to protect money, accounts, and evidence."]
                ].map(([title, text]) => (
                  <article key={title}>
                    <span>✓</span>
                    <h3>{title}</h3>
