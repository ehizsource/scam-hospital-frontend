...
const PACKAGES = [
  {
    ...
    dark: false,
    popular: false,
    features: ["Submit scam message", "Full detailed analysis report", "Scam category detection"]
  },
  {
    name: "Basic",
...
function formatSlotForCountry(date, time, timeZone) {
  ...
  return formatter.format(getDateTimeFromSlot(date, time))
}
...
export default function App({ onBack = () => {} }) {
  ...
  const timezoneLabel = form.country ? selectedTimeZone.replace("_", " ") : "GMT"
  ...
  const updateCountry = (country) => {
    ...
    setCurrency(COUNTRY_CURRENCY[country] || "USD")
    setCountrySearch("")
  ...
  const isTimeBooked = (date, time) => {
    return BOOKED_SLOTS[date] && BOOKED_SLOTS[date].includes(time)
  }
  ...
  const renderCalendar = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      const fullyBooked = (BOOKED_SLOTS[dateStr] || []).length >= TIMES.length
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      alert("Could not connect to backend. Start FastAPI with: uvicorn main:app --reload")
    }
  }
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      alert("Your backend needs an /initialize-payment route before Paystack can work.")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="app-shell">
      ...
      <div className="progress">
        {["Choose Package", "Your Details", "Payment"].map((label, index) => (
        ...
        {step === 2 && (
          <section className="panel">
              ...
              <div>
                <div className="field-label">
                  ...
                  {autoDetectedScamType && (
                    <span className="detect-badge">Auto-detected: {autoDetectedScamType}</span>
                  )}
                </div>
            ...
            <label className="field-label" htmlFor="description">
              ...
              <span className="field-hint">Auto-detection enabled</span>
            </label>
            <textarea
              id="description"
              name="description"
            ...
            {form.date && (
              <div className="time-slots">
                <div className="local-time-banner">
                  <div>
                    <span className="local-time-kicker">Smart local booking</span>
                    <h2>Select Time Slot</h2>
                  </div>
                  <p>
                    {form.country
                      ? `Your appointment options are tailored to ${form.country}, so the time you choose is the time you can plan around.`
                      : "Choose your country first and we will tailor the appointment times to your location."}
                  </p>
                  {form.country && <strong>{timezoneLabel}</strong>}
                </div>
                <div className="button-row">
                  {TIMES.map((time) => {
                    const booked = isTimeBooked(form.date, time)
                    const localTime = formatSlotForCountry(form.date, time, selectedTimeZone)
                    return (
                      <button
                        type="button"
                        key={time}
        ...
        {step === 3 && (
          <section className="panel">
            <div className="panel-header">
              ...
              <p>Review the appointment information before payment.</p>
            </div>
            ...
            <div className="actions">
              ...
              <button type="button" onClick={handlePay} disabled={loading} className="primary-button">
                {loading ? "Processing..." : `Pay ${getPrice(selected?.priceUSD)} with Paystack`}
              </button>
            </div>
          </section>
        )}
